...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
	...
	Q_FOREACH(const SendCoinsRecipient& rcp, recipients) {
		...
		else { // User-entered aitra address / amount:
...

	{
		...
		if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
			Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 AITRA.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
			...
			return TransactionCreationFailed;
		        ...
		        bool fCreated = wallet->CreateTransaction(vecSend,
                                                  ...
                                                  *keyChange,
                                                  ...
                                                  true,
                                                  recipients[0].useSwiftTX ? true : false,
                                                  0);
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

	{
		...
		Q_FOREACH(const SendCoinsRecipient& rcp, recipients) {
			...
			else if (!rcp.message.isEmpty()) // Message from normal aitra:URI (aitra:XyZ...?message=example)
			{
...
bool WalletModel::isUsed(const CTxDestination& address)
{
