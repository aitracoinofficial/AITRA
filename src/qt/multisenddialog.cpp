...
void MultiSendDialog::on_addButton_clicked()
{
	...
	CTxDestination strAddress = DecodeDestination(ui->multiSendAddressEdit->text().toStdString());
    if (!IsValidDestination(strAddress)) {
    ...
    std::pair<std::string, int> pMultiSend;
    pMultiSend.first = EncodeDestination(strAddress);
    ...
    if (model && model->getAddressTableModel()) {
        ...
        if (!userInputLabel.empty())
            model->updateAddressBookLabels(strAddress, userInputLabel, "send");
        else
            model->updateAddressBookLabels(strAddress, "(no label)", "send");
    }
