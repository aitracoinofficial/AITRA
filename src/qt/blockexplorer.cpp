...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::AITRA, nValue);
...
{
    std::string genesisblockhash = "00000efbbd4125b8441b5e3b97eb7f6eaa8a8a6c18e5a1dd2fde6969d1a31917";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (aitra.conf).");
            QMessageBox::warning(this, "AITRA Coin Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
