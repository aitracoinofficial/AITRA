...
bool CAitraStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CAitraStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAitraStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CAitraStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CAitraStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CAitraStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CAitraStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CAitraStake::GetUniqueness() const
{
    //The unique identifier for a AITRA stake is the outpoint
...
CBlockIndex* CAitraStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CAitraStake::ContextCheck(int nHeight, uint32_t nTime)
{
