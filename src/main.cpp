...
#if defined(NDEBUG)
#error "AITRA_Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uaitra) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateAITRASupply(int nHeightStart)
{
	...
	uiInterface.ShowProgress(_("Recalculating AITRA supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating AITRA supply..."), percent);
		}
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 71205920 && nHeight > 200000) {
		nSubsidy = 5 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
{
    util::ThreadRename("aitra-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> aitraInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                aitraInputs.push_back(stakeIn);
        }
        const bool hasAITRAInputs = !aitraInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasAITRAInputs) {
                    ...
                    for (const CTxIn& aitraIn : aitraInputs)
                        if(aitraIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasAITRAInputs) {
                            for (const CTxIn& stakeIn : aitraInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // AITRA_Coin: We use certain sporks during IBD, so check to see if they are
