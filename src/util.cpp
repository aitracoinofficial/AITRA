...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\AITRA_Coin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\AITRA_Coin
// Mac: ~/Library/Application Support/AITRA_Coin
// Unix: ~/.aitra
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AITRA";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "aitra.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty aitra.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override aitra.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "aitrad.pid"));
